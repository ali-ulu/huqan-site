...
Commit `f7a1536` GitHub `main` dalına gönderildi. GitHub Actions `pages build and deployment` çalışması aynı SHA için `in_progress`; GitHub Pages API `status: building` döndürüyor. Bu nedenle `https://huqan.com/blog/` kontrolü deployment tamamlanmadan önce eski canlı homepage/fallback içeriğini gösterdi. Yerel sunucu tüm yeni rotaları 200 ile servis ediyor. Deployment bitince canlı cache ve rota doğrulaması tekrarlanmalıdır.

Canlı `https://huqan.com/robots.txt` deployment sonrası site reposundaki kurallardan önce Cloudflare Managed Content bloğunu döndürüyor. Bu blok `Content-Signal: search=yes,ai-train=no,use=reference` yayıyor; ayrıca `GPTBot`, `Google-Extended`, `ClaudeBot` ve bazı diğer crawler'lar için `Disallow: /` içeriyor. `OAI-SearchBot` ve `PerplexityBot` için Cloudflare bloğunda ayrı bir Disallow görünmüyor; Googlebot/Bingbot ve repo kurallarında Allow var. Bu nedenle repo tarafı crawl/sitemap altyapısı tamamlanmış olsa da ChatGPT/OpenAI ve Google Extended görünürlüğünün bir bölümü Cloudflare hesabındaki robots/content-signal ayarları tarafından yönetiliyor; Cloudflare panelinde izinler değiştirilmeden tamamen açılmaz.

Deployment sonrası canlı `/sitemap.xml` 200 ile açıldı ve English/Turkish blog index, üç ana içerik ve yeni NeMo alternatives URL'lerini içeriyor. Canlı `/llms.txt` 200 ile açıldı; kanonik URL'ler, desteklenen yetenekler, non-claims ve kaynak tercihleri public olarak okunabiliyor. Canlı `/blog/` ve `/blog/nemo-guardrails-alternatives/` sayfaları da yeni içerikle eşleşiyor.

Son Pages deployment (`e268a8e`) sonrası canlı `/blog/what-is-ai-agent-governance/` ve `/tr/blog/best-ai-governance-tools/` sayfaları query cache-buster ile tekrar açıldı. Her ikisi de doğru title, H1, kısa cevap, Türkçe/İngilizce içerik ve FAQ etkileşim yüzeylerini gösterdi. Türkçe listicle'da dört maddelik kısa cevap kutusu ve beş araç navigasyonu görünür.
